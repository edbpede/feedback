...
<div class="space-y-4">
  ...
  <div class="space-y-2">
    {#each DECLINE_OPTIONS as option (option.reason)}
      <button
        type="button"
      ...
      >
        <span class="{option.icon} text-muted-foreground text-xl"></span>
        <span class="font-medium">{t(option.labelKey as Parameters<typeof t>[0])}</span>
        <span class="i-carbon-chevron-right text-muted-foreground ml-auto"></span>
      </button>
    {/each}
  </div>
